...
> **Diagram placeholder (request lifecycle).** Show: client request to the gateway; authentication and rate-limit check; model and privacy-mode resolution from the model ID; billing hold placed; provider call inside the selected privacy boundary; response (streamed or whole) returned; hold captured on success or released on failure.
