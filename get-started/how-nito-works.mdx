...
If you read only one section, read [The request lifecycle](#the-request-lifecycle). Everything else expands on it.
...
You send a familiar chat or embeddings request to Nito; the model ID you put in the request selects both the model and how private the call is; Nito routes, bills, and runs it; and for the highest tiers it gives you an attestation (a signed statement about the sealed environment that ran your call) that you can verify independently.

Hold onto one idea from that sentence: **the model ID carries two decisions at once**, which model and how private. That is the hinge the whole product turns on, and we come back to it below.

## The request lifecycle

> **Diagram placeholder (request lifecycle).** Show: client request to the gateway; authentication and rate-limit check; model and privacy-mode resolution from the model ID; billing hold placed; provider call inside the selected privacy boundary; response (streamed or whole) returned; hold captured on success or released on failure. Marked TODO for design.

Here is what actually happens when you make a call, step by step. You only ever touch steps 1 and 2 directly; the rest is the gateway doing its job, but knowing them helps you reason about errors, cost, and latency.

1. **Authentication.** You send your API key as a bearer token in the `Authorization` header. Nito looks it up, resolves it to your organization (the account that owns billing and keys), and rejects the request with `401` if the key is missing, malformed, revoked, or expired. This is also where rate limits are checked: if you are over a limit, you get `429` with a `Retry-After` header telling you how long to wait.

2. **Model and privacy resolution.** Nito reads the `model` field. The base part selects the model; an optional suffix selects a stronger privacy mode. `grok-4.3` is ordinary mode; `grok-4.3:tee` asks for execution inside a Trusted Execution Environment; `grok-4.3:e2e-tee` asks for end-to-end encrypted TEE. Only `:tee` and `:e2e-tee` are valid suffixes, and the chosen model must actually support the mode you ask for, or the call is rejected rather than quietly downgraded. This refusal-to-downgrade is deliberate: a privacy guarantee that silently weakens is worse than no guarantee.

3. **Routing.** A single logical model can be served by more than one physical backend (the same model at different providers, or at different privacy tiers). Nito gathers the eligible backends and chooses one, filtering first by hard requirements (your privacy mode, the features you asked for, the inputs you sent) and then ordering what remains by availability and cost. The result is that you name one model and Nito quietly picks a healthy, eligible, cost-effective backend to run it.

4. **Billing hold.** Before spending real money on a provider call, Nito places a hold against your credit balance, sized to the expected cost of the request. A hold is like the pre-authorization a hotel puts on a card at check-in: it reserves the funds without charging them yet. This guarantees you can pay for the work before it starts.

5. **Inference.** Nito calls the chosen provider inside the privacy boundary you selected, and returns the result either as one complete response or as a stream of tokens (small incremental pieces) over a long-lived connection.

6. **Settlement.** When the call succeeds, the hold is captured for the actual usage (which may be less than the estimate). If the request fails before producing any usage, the hold is released and you are not charged. This is why a failed call does not cost you credits, and it is a direct consequence of the hold-then-settle design in step 4.

A useful way to remember it: **authenticate, resolve, route, hold, run, settle.** Two of those (resolve and run) are shaped by the single `model` string you send.
...
Each entry includes the model `id` you pass back in a request, its `type` (chat, embedding, image), its `capabilities` (which features it supports), and its `privacy_tier`. One detail matters for how you write code: **the catalog is discovered from providers and kept current, not hard-coded.** Models are added and retired over time. So treat `GET /v1/models` as the live source of truth and select models by capability at runtime, rather than pasting a model name into your source and hoping it still exists next quarter.
...
This is the part that makes Nito different, so it is worth slowing down.
...
| `model-id:tee` | TEE | Runs inside a Trusted Execution Environment: a hardware-isolated, sealed compute box the host cannot peer into. You can fetch an attestation and verify the call really ran there. |
| `model-id:e2e-tee` | End-to-end encrypted TEE | Adds client-side encryption on top of TEE, so the data is encrypted to the sealed environment itself. The strongest mode. It does not stream and supports fewer features, by design. |
...
Which modes a given model supports is reported on its catalog entry (look at `privacy_tier`, `attestation_available`, and `e2ee_supported`). TEE and end-to-end TEE come from specific providers only. The full treatment is in [Per-call privacy modes](/privacy/per-call-privacy-modes) and [Privacy tiers in depth](/privacy/tiers).

<Warning>
Privacy modes are selected by the model ID suffix, and the only valid suffixes are `:tee` and `:e2e-tee`. Any other suffix (`:standard`, `:zdr`, `:private`, and so on) is rejected. There is no separate `tier` field in the request body. If you want a different privacy mode, you change the model string, nothing else.
</Warning>
...
- **It does not keep your conversation history by default.** Where a chat history exists (for example in a browser app), it is stored on the client side. The gateway's default posture is metadata only. See the [Privacy overview](/privacy/overview) for the precise contract.
...
- **"`:tee` will fall back to a normal backend if no TEE is free."** No. A privacy mode never silently downgrades. If it cannot be served as asked, the call fails.
...
- Make a real call: **[60-second quickstart](/get-started/quickstart-60-seconds)**.
- Understand keys first: **[Authentication](/get-started/authentication)**.
- Go deep on the privacy dial: **[Privacy overview](/privacy/overview)**.

{/* source: internal/server/routes.go, internal/models/models.go (ParseLogicalModelIDForPublicMode, PublicPrivacyMode*), internal/server/logical_models.go (chooseLogicalRoute), internal/billing/enforcement.go (AuthorizeInference, hold lifecycle), internal/billing/state_machine.go, internal/server/api_models.go (apiModelsHandler), internal/server/rate_limit.go (Retry-After) */}
