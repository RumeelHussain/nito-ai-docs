...
2. **Model and privacy resolution.** Nito reads the `model` field. The base part selects the model; an optional suffix selects a stronger privacy mode. `grok-4.3` is ordinary mode; `grok-4.3:confidential` asks for execution inside a Trusted Execution Environment; `grok-4.3:encrypted` asks for end-to-end encrypted TEE. Only `:confidential` and `:encrypted` are valid suffixes, and the chosen model must actually support the mode you ask for, or the call is rejected rather than quietly downgraded. This refusal-to-downgrade is deliberate: a privacy guarantee that silently weakens is worse than no guarantee.
