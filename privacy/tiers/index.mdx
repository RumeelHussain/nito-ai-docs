...
<Note>
Confidential and Encrypted were previously called `tee` and `e2e-tee`. The old `:tee` / `:e2e-tee` suffixes and `tee` / `e2e-tee` `privacy_tier` values still work as deprecated aliases, so existing integrations keep running.
</Note>
